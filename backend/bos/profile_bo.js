...
// Follow profile
module.exports.followProfile = function(params, res) {
    var insertParameters = {};
    dao.executeQuery('select profile_id, user_id from profile_details, account_details where account = user_id and unique_id = ?', [params.unique_id], function(profile_results) {
        if (exists(profile_results[0].profile_id)) {
            insertParameters.profile = profile_results[0].profile_id;

						if (exists(params.following_id)) {
                insertParameters.following = params.following_id;

								dao.insertData('follow_details', insertParameters, function(follow_result) {
						...
						res.send({
								"status_code": 500,
								"message": "'profile_id' does not exist."
						});
				}
