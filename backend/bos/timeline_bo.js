			...
			if (connection_result[0].count > 0) {
				dao.executeQuery('select * from post_details, profile_details where profile_id = profile and profile = ?', [friend], function(post_result) {
					...
					for (var i = 0; i < post_result.length; i++) {
						...
						if (post_result[i].profile_pic != 0 && photo_ids.indexOf(post_result[i].profile_pic) === -1) {
							photo_ids.push(new ObjectID(post_result[i].profile_pic))
						}
						if (post_result[i].photo != 0 && photo_ids.indexOf(post_result[i].photo) === -1) {
							photo_ids.push(new ObjectID(post_result[i].photo))
						}
					}
					db.get("photos").find({
						'_id': {
						...
						for (var k = 0; k < post_result.length; k++) {
							if (exists(photo_results[post_result[k].profile_pic])) {
								post_result[k].profile_pic = photo_results[post_result[k].profile_pic];
							}
							if (exists(photo_results[post_result[k].photo])) {
								post_result[k].photo = photo_results[post_result[k].photo];
							}
						}
		...
		profile_bo.fetchReceivers(profile, function(receiver_results) {
			var notifyFunction = function(i) {
				return function(callback) {
					notification_bo.notify(receiver_results[i].receiver,
